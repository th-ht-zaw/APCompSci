...
{ 
     ...
     *****************************************************/
    //MUST BE PRIVATE -- You should not be able to see this so you need a wrapper
	...
	public MyIterator() 
	{
	    _dummy = _head; //Should be _dummy = new DLLNode<T>(null, null, head); 
	    _okToRemove = false;
	}
	...
	public boolean hasNext() 
	{
	    ...
	    return false; //Collapse into _dummy.getNext() != null;
	}
	...
	public T next() 
	{
	    ...
	    T holder = _dummy.getNext().getCargo();
	...
	public void remove() 
	{
	    ...
	    //if ________...
	    if ( _dummy.getCargo() == getFirst() ) {
		removeFirst();
		_dummy = _head;
	    }
	    //if ________...
	    else if ( _dummy.getCargo() == getLast() ) {
		removeLast();
		_dummy = _tail;
	    }
	    else if ( size() == 1 ) {
		...
		DLLNode<T> holder = _dummy.getPrev();
		holder.setNext(_dummy.getNext());
		_dummy.getNext().setPrev(holder);
		_dummy = holder; 
	    }
	...
	MyIterator tester = new MyIterator();
	return tester;
	//OR return new MyIterator(); 
    } 
